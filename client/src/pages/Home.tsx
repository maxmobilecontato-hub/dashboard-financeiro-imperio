...
import { buildMonthlyProfitComparison, buildMonthlyRevenueComparison, calculateProfit, extractInvoiceTotalText, getBillStatus, isValidPdfFile, isValidXlsxFile, parseReceivablesRows, percentChange, rankByName } from "@/lib/financeRules";
...
type PdfRecord = { name: string; importedAt: string; category: "financeiro" | "nota"; total: number; textAvailable: boolean };
...
export default function Home() {
  ...
  const [pdfRecords, setPdfRecords] = useState<PdfRecord[]>(() => { try { return JSON.parse(localStorage.getItem("finance-dashboard-pdfs") || "[]"); } catch { return []; } });
  ...
  const invoiceTotal = pdfRecords.filter((item) => item.category === "nota").reduce((sum, item) => sum + item.total, 0);
  ...
  async function handlePdf(file: File | undefined, category: PdfRecord["category"]) { if (!file) return; setError(""); if (!isValidPdfFile(file.name)) { setError("Formato inválido. Envie somente um arquivo PDF."); return; } setLoading(true); try { const text = await extractPdfText(file); const record: PdfRecord = { name: file.name, importedAt: new Date().toLocaleString("pt-BR", { dateStyle: "short", timeStyle: "short" }), category, total: category === "nota" ? extractInvoiceTotalText(text) : 0, textAvailable: text.length > 0 }; const next = [record, ...pdfRecords.filter((item) => item.name !== record.name)].slice(0, 20); setPdfRecords(next); localStorage.setItem("finance-dashboard-pdfs", JSON.stringify(next)); } catch { setError("Não foi possível ler este PDF. PDFs escaneados podem precisar de OCR antes da leitura."); } finally { setLoading(false); } }
